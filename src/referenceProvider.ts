...
import { codeLensMap,clsLangId, intLangId, macLangId, lsExtensionId, objectScriptApi } from "./extension";
    ...
    async provideReferences(document: vscode.TextDocument, 
        ...
        if (className) {
            ...
            if (document.languageId===clsLangId) {
                ...
                if (memberName==='') {
                    console.log("not able to find symbol (cursor must be on the top line of the method");
                } else {
                ...
                if (respdata2 !== undefined && respdata2.data.status.errors.length === 0 && respdata2.data.result.content.length > 0) {
                    ...
                    for (let memobj of respdata2.data.result.content) {
                       ...
                       const uri=objectScriptApi.getUriForDocument(`${memobj.CalledByKey1}.cls`);
                       const line=parseInt(memobj.LineNumber);
                       result.push(new vscode.Location(uri,new vscode.Position(line-1,1)));
                    }        
                } else {
